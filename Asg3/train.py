...
def train(epoch, data_loader):
    ...
    for data in tqdm(data_loader, ascii=True):
        ...
        output = model(input)  # Step 2: Forward pass
        # print(f"Output shape: {output.shape}, Target shape: {target.shape}")
        loss = loss_function(output, target)  # Step 3: Compute loss
        loss.backward()  # Step 4: Backward pass
        optimizer.step()  # Step 5: Update model parameters
        loss_metric.add(loss.item())  # Update the loss metric
    avg_loss = loss_metric.average()  # Calculate the average loss
